...
engine.execute(text("CREATE TABLE IF NOT EXISTS predict_batter_stats(game_date TEXT, game_id TEXT, player_id TEXT, player_name TEXT, team TEXT, role TEXT, atBats float8, avg float8, baseOnBalls float8, doubles float8, hits float8, homeRuns float8, obp float8, ops float8, rbi float8, runs float8, slg float8, strikeOuts float8, triples float8, singles float8, difficulty float8);"))
engine.execute(text("CREATE TABLE IF NOT EXISTS predict_pitcher_stats(game_date TEXT, game_id TEXT, player_id TEXT, player_name TEXT, team TEXT, role TEXT, atBats float8, baseOnBalls float8, blownsaves float8, doubles float8, earnedRuns float8, era float8, hits float8, holds float8, homeRuns float8, inningsPitched float8, losses float8, pitchesThrown float8, rbi float8, runs float8, strikeOuts float8, strikes float8, triples float8, whip float8, wins float8, difficulty float8);"))
