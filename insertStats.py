    ...
    if role == 'BatterData':
        ...
        for index, row in data.iterrows():
            engine.execute(text(f"INSERT INTO batter_stats(game_id, game_date, player_id, career_atBats, career_avg, career_homeRuns, career_obp, career_ops, career_rbi, career_slg, career_strikeOuts, recent_atBats, recent_avg, recent_homeRuns, recent_obp, recent_ops, recent_rbi, recent_slg, recent_strikeOuts) \
                                VALUES('{gameid}', '{gamedate}', '{int(row['player_id'])}', '{round(row['career_atBats'], 3)}', '{round(row['career_avg'])}', '{round(row['career_homeRuns'])}', '{round(row['career_obp'])}', '{round(row['career_ops'])}', '{round(row['career_rbi'])}', '{round(row['career_slg'])}', '{round(row['career_strikeOuts'])}', '{round(row['recent_atBats'])}', '{round(row['recent_avg'])}', '{round(row['recent_homeRuns'])}', '{round(row['recent_obp'])}', '{round(row['recent_ops'])}', '{round(row['recent_rbi'])}', '{round(row['recent_slg'])}', '{round(row['recent_strikeOuts'])}');"))
    ...
    if role == 'PitcherData':
        ...
        for index, row in pitchers.iterrows():
            ...
            engine.execute(text(f"INSERT INTO pitcher_stats(game_id, game_date, player_id, career_era, career_homeRuns, career_whip, career_battersFaced, recent_era, recent_homeRuns, recent_whip, recent_battersFaced) \
                                VALUES('{gameid}', '{gamedate}', '{int(row['player_id'])}', '{round(row['career_era'])}', '{round(row['career_homeRuns'])}', '{round(row['career_whip'])}', '{round(row['career_battersFaced'])}', '{round(row['recent_era'])}', '{round(row['recent_homeRuns'])}', '{round(row['recent_whip'])}', '{round(row['recent_battersFaced'])}');"))