...
def is_past_game_item(player_game, main_game_nhl_id):
    return player_game.get('gameid', 0) < main_game_nhl_id

def sort_by_nhl_id(game_item_a, game_item_b):
    return game_item_a['gameid'] - game_item_b['gameid']
...
def get_skater_historical_stats(player_id, engine):
    ...
    sakter_items = [{'gameid':skater['game_id'], 'season_id':skater['season_id'], 'stats':{'position':skater['position'], 'goals':skater['goals'], 'assists':skater['assists'], 'points':skater['points'], 'plus_mius':skater['plus_mius'], 'penalty_minutes':skater['penalty_minutes'], 'time_on_ice':skater['time_on_ice'], 'corsi_for':skater['corsi_for'], 'corsi_against':skater['corsi_against'], 'fenwick_for_percent':skater['fenwick_for_percent'], 'fenwick_for_percent_relative':skater['fenwick_for_percent_relative']}} for skater in skater_stats]
...
def get_recent_form(past_sorted_game_items):
    ...
    def apply_recent_form_weights(stats_set, index):
        ...
        for stat_name, stat_value in stats_set.items():
            if isinstance(float(stat_value), (int, float)):
                stats_set[stat_name] = float(stats_set[stat_name]) * weight
        return stats_set
...
def generate_model_input(gameId, engine):
    ...
    if skater_stats == [] or goaltender_stats == []:   
        ...
        sakter_items = [{'player_id':skater['player_id'], 'team_side':skater['team_side'], 'stats':{'position':skater['position'], 'goals':skater['goals'], 'assists':skater['assists'], 'points':skater['points'], 'plus_mius':skater['plus_mius'], 'penalty_minutes':skater['penalty_minutes'], 'time_on_ice':skater['time_on_ice'], 'corsi_for':skater['corsi_for'], 'corsi_against':skater['corsi_against'], 'fenwick_for_percent':skater['fenwick_for_percent'], 'fenwick_for_percent_relative':skater['fenwick_for_percent_relative']}} for skater in skater_stats]
