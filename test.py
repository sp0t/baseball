...
away_starter = ''
home_starter = ''
batters = pd.read_sql(text(f"SELECT * FROM batter_table WHERE game_id = '717361' and substitution = '0' ORDER BY team, position;"), con = engine).to_dict('records')
for batter in batters:
    if batter['team'] == 'away':
...
pithcers = pd.read_sql(text(f"SELECT * FROM pitcher_table WHERE game_id = '717361' AND role = 'starter';"), con = engine).to_dict('records')
...
rosters = schedule.get_rosters('717361')

save_batter_data(engine, X_test, awaybatters, homebatters, '717361', rosters)
save_pitcher_data(engine, X_test, away_starter, home_starter, '717361', rosters)
...
engine.execute(f"INSERT INTO win_percent_c(game_id, away_prob, home_prob) VALUES('{717381}', '{pred_1a[0]}', '{pred_1a[1]}') ON CONFLICT (game_id) DO UPDATE SET away_prob = excluded.away_prob, home_prob = excluded.home_prob;")  
